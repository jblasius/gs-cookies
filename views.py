...
def index():
    ...
    if current_user.is_authenticated:
        # Define the location for the configuration cache
        cc = FileSystemCache('/var/www/html/gs-cookies/cache/config',
                             default_timeout=300)

        # Determine if the cache is still valid. If not, refresh it.
        if cc.get('first_monday') is None:
            configuration = get_configuration()

        todays_date = datetime(2017, 3, 17)

        week_num = get_week_num(todays_date, cc)

        week1_doc = "https://docs.google.com/document/d/e/2PACX-1vQheEcqevUqMXSYLCjr73wcc9NhiCe0Rx0JE6MBPkMym2oYZ9FEscs_iIeZbqNT7gCXYUNYZr2cAqK5/pub?embedded=true"
        week2_doc = "https://docs.google.com/document/d/e/2PACX-1vQx9vO2Tn_FpVlRwohqPDUi0WYBfLRXBRpZsou98DNvcWxzwtKVWWgHtTarxb-sVy0asSU2URWk8Pua/pub?embedded=true"
        week3_doc = "https://docs.google.com/document/d/e/2PACX-1vQQXGpg3i8zT7OovgVUuVYHRGm6z29OdgcENWTIuOKjOGHio31GdWk-ZefOZ7x9wTUhSEm5XNdc0soO/pub?embedded=true"
        week4_doc = "https://docs.google.com/document/d/e/2PACX-1vRfjbziBzsJHG4O-tGydlWVBF59iQEiTa3LCCYVGS_GL7MZUCYC5YM-myKDBWHmreRcjvV_EQSoA3iL/pub?embedded=true"
        week5_doc = "https://docs.google.com/document/d/e/2PACX-1vSoAMUVSLRitUZCfmI6xsaoBLGm22wN6BpUKdCYGKrtniEGHzpL_9nnjczcyNHpLJc4yg0ijupxvY0u/pub?embedded=true"
        week6_doc = "https://docs.google.com/document/d/e/2PACX-1vTmPgIleUA9PDOlYomntt06SNlpur8TLJmAgYYKosJALNHaWqGBLQ_IoCf_BC-hpZ18h3ByMOcl2Ehm/pub?embedded=true"
        week7_doc = "https://docs.google.com/document/d/e/2PACX-1vSBrKsotopLzZ4cm2ClQAJzUGKzRdAtJnYZ75BhjdTU877yuWycexBMVNiI72-4kwFE_mGFID91Vr2c/pub?embedded=true"
        week8_doc = "https://docs.google.com/document/d/e/2PACX-1vTA005M00V5hrGJ3bcKucfog16pPSL5H9ecvuIe5OlzMNIqBN4vWHOxd4dnUSK0j0atd6nxWlGpk0SJ/pub?embedded=true"
        week9_doc = "https://docs.google.com/document/d/e/2PACX-1vQW5Di4gLccVOL_DuTae0vtFrz3xeOQKWh9eBL4gcxfgYC1o_vGmu9P58Gbh_tyoOvoybY0RbchHjBk/pub?embedded=true"
...
def goal():
    ...
    # Initialize the totalizer dictionary items
    girl_total = {}
    girl_total['sold'] = 0.0
    ...
    all_transactions = get_all_transactions(todays_date)
    ...
    for transaction in all_transactions:
        if transaction[3] == session['girl_name']: # If the transaction belongs to the current girl, totalize it
            girl_goc['sold'] += float(transaction[12] or 0) #number as a string
            girl_total['sold'] += float(transaction[13] or 0) #number as a string

            # Loop through weeks 1-current week and totalize the goc, personal, and booth numbers
            for i in xrange(1, week_num+1):
                if transaction[18] == str(i):
                    week = 'week' + str(i)
                    girl_goc[week] += float(transaction[12] or 0)
                    if transaction[1] == "Personal":
                        girl_total[week+'_personal'] += float(transaction[13] or 0)
                        exit
                    if transaction[1] == "Booth":
                        girl_total[week+'_booth'] += float(transaction[13] or 0)
                        exit
            
        if transaction[17] == girl_info['troop']: # If the transaction belongs to the same troop totalize it
            troop_goc['sold'] += float(transaction[12] or 0) #number as a string
            troop_total['sold'] += float(transaction[13] or 0) #number as a string

            # Loop through weeks 1-current week and totalize the goc, personal, and booth numbers
            for j in xrange(1, week_num+1):
                if transaction[18] == str(j):
                    week = 'week' + str(j)
                    troop_goc[week] += float(transaction[12] or 0)
                    if transaction[1] == "Personal":
                        troop_total[week+'_personal'] += float(transaction[13] or 0)
                        exit
                    if transaction[1] == "Booth":
                        troop_total[week+'_booth'] += float(transaction[13] or 0)
                        exit

    # Create the cumulatize totals for each week for the line charts
    ...
    if week_num == 1:
        yellowFrom = float(cc.get('week1_yellowFrom'))
        yellowTo = float(cc.get('week1_yellowTo'))
    elif week_num == 2:
        yellowFrom = float(cc.get('week2_yellowFrom'))
        yellowTo = float(cc.get('week2_yellowTo'))
    elif week_num == 3:
        yellowFrom = float(cc.get('week3_yellowFrom'))
        yellowTo = float(cc.get('week3_yellowTo'))
    elif week_num == 4:
        yellowFrom = float(cc.get('week4_yellowFrom'))
        yellowTo = float(cc.get('week4_yellowTo'))
    elif week_num == 5:
        yellowFrom = float(cc.get('week5_yellowFrom'))
        yellowTo = float(cc.get('week5_yellowTo'))
    elif week_num == 6:
        yellowFrom = float(cc.get('week6_yellowFrom'))
        yellowTo = float(cc.get('week6_yellowTo'))
    elif week_num == 7:
        yellowFrom = float(cc.get('week7_yellowFrom'))
        yellowTo = float(cc.get('week7_yellowTo'))
    elif week_num == 8:
        yellowFrom = float(cc.get('week8_yellowFrom'))
        yellowTo = float(cc.get('week8_yellowTo'))
    elif week_num > 8:
        yellowFrom = float(cc.get('week9_yellowFrom'))
        yellowTo = float(cc.get('week9_yellowTo'))
...
def financials():
    ...
    todays_date = datetime(2017, 3, 1)

    week_num = get_week_num(todays_date, cc)

    transactions = get_girl_transactions(session['girl_name'], todays_date)
    ...
    for personal_transaction in personal_transactions:
        total_sold += float(personal_transaction[13] or 0) #zero based, nummer as a string
        total_due += Decimal(sub(r'[^\d\-.]', '', personal_transaction[14])) #zero-based
        total_paid += Decimal(sub(r'[^\d\-.]', '', personal_transaction[15])) #zero-based
    ...
    info['girl_name'] = session['girl_name']

    totals = {}
    totals['due'] = total_due
    totals['paid'] = total_paid
    totals['remain'] = total_due - total_paid
...
def incentives():
    ...
    todays_date = datetime(2018, 2, 17)
    ...
    for key, value in reward_choices.iteritems():
        ...
        match = re.search(r'Program Credits', value)
        if match:
            this_pic = reward_pics['Program Credits']
        elif value == '#REF!':
            this_pic = ''
        else:
            try:
...
@login_required
def downloads():
    folder = "0BzC5F6zg91g4QjZWQ0hJbEFaOUk"
